...
function onProposeTrade(tx) {
  ...
  return getAssetRegistry('org.sample.Animal')
    .then(function (ar) {
      // update the Animal in the registry
      return ar.update(tx.animal);
    })
}
...
function onAcceptTrade(tx) {
  ...
  return getParticipantRegistry('org.sample.Farmer')
    .then(function (fr) {
      farmerRegistry = fr;
      // find the proper farmer in the registry
      return fr.get(getCurrentParticipant().getIdentifier());
    })
    .then(function (f) {
      farmer = f;
      ...
      return farmerRegistry.update(farmer);
    })
    .then(function () {
      // get animals registry
      return getAssetRegistry('org.sample.Animal');
